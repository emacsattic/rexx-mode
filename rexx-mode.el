...
(setq rexx-font-lock-keywords-2
  (append
    rexx-font-lock-keywords-1
    (list
      (list
	(concat
	  "\\<\\(a\\(ddress\\|rg\\)\\|break\\|call\\|d\\(o\\|rop\\)\\|"
	  "e\\(cho\\|lse\\|nd\\|xit\\)\\|i\\(f\\|nterpret\\terate\\)\\|"
	  "forever\\|leave\\|n\\(op\\|umeric\\)\\|o\\(ptions\\|therwise\\)\\|"
	  "p\\(arse\\|rocedure\\|u\\(sh\\|ll\\)\\)\\|queue\\|return\\|"
	  "s\\(ay\\|elect\\|hell\\|ignal\\)\\|t\\(hen\\|o\\|race\\)\\|"
	  "u\\(ntil\\|pper\\)\\|wh\\(en\\|ile\\)\\)\\>"
	  )
	'(1 font-lock-keyword-face nil))
      (list
	(concat
	  "\\<\\("
	  "a\\(b\\(brev\\|s\\)\\|dd\\(lib\\|ress\\)\\)\\|"
	  "b\\(2c\\|it\\(c\\(hg\\|lr\\|omp\\)\\|or\\|set\\|tst\\|xor\\)\\)\\|"
	  "c\\(2\\(b\\|d\\|x\\)\\|ent\\(re\\|er\\)\\|"
	  "lose\\|o\\(mp\\(are\\|ress\\)\\|pies\\)\\)\\|"
	  "d\\(at\\(atype\\|e\\)\\|el\\(str\\|word\\)\\|igits\\)\\|"
	  "e\\(of\\|rrortext\\|x\\(port\\|ists\\)\\)\\|"
	  "f\\(orm\\|reespace\\|uzz\\)\\|get\\(clip\\|space\\)\\|hash\\|"
	  "i\\(mport\\|n\\(dex\\|sert\\)\\)\\|"
	  "l\\(astpos\\|e\\(ft\\|ngth\\)\\|ines\\)\\|"
	  "m\\(ax\\|in\\)\\|o\\(ffset\\|pen\\|verlay\\)\\|p\\(os\\|ragma\\)\\|"
	  "r\\(and\\(om\\|u\\)\\|e\\(ad\\(ch\\|ln\\)\\|mlib\\|verse\\)\\|ight\\)\\|"
	  "s\\(e\\(ek\\|tclip\\)\\|how\\|ign\\|ourceline||\pace\\|"
	  "t\\(\\orage\\|rip\\)\\|ub\\(str\\|word\\)\\|ymbol\\)\\|"
	  "t\\(ime\\|\\(r\\(a\\(ce\\|nslate\\)\\|im\\|unc\\)\\)\\)\\|"
	  "upper\\|v\\(alue\\|erify\\)\\|"
	  "w\\(ord\\(\\|index\\lengh\\|s\\)\\|rite\\(ch\\|ln\\)\\)\\|"
	  "x\\(2c\\|range\\)"
	  "\\)\\>"
	  )
...
;; Level 3 -  ports
(setq rexx-font-lock-keywords-3
  (append
    rexx-font-lock-keywords-2
    (list
      ;; ports
      (list
